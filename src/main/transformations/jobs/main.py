...
logger.info("Connection to MySQL successfully created")
...
if csv_files:
    ...
    if data:
        logger.info("Your last process was incomplete or failed")
    else:
...
logger.info(f"List of file present at local directory after download are: {all_files}")
...
# Move the data to error direcrtory on local
...
if error_files:
    for file_path in error_files:
        if os.path.exists(file_path):
            ...
            logger.info(f"Moved '{file_name}' for s3 file path to '{destination_path}.'")
        ...
        logger.info("************** There is no error files available at out dataset ***********")
...
if correct_files:
    ...
    logger.info("************** Connection with MySql server ********")
    connection = get_mysql_connection()
    cursor = connection.cursor()
...
another datamart for customer who bought hum much each days of month
for every month there should be a file and inside that there should be a store_id segregation
Read the data from parquet and generate csv
in which there will be a sales_person_name, sales_person_store_id, sales_person_total_billing_done_for_each_month
...
logger.info("************************** Loading sales table into sales_team_table  **************************")
sales_team_table_df = database_client.create_dataframe(spark, config.sales_team_table)

logger.info("************************** Loading store table into store_tabl  **************************")
...
file will be written to local files
...
final_customer_data_mart_df = s3_customer_store_sales_df_join.select(
    "ct.customer_id", "ct.first_name", "ct.last_name", "ct.address", "ct.pincode", "ct.phone_number", "sales_date", "total_cost"
)
...
logger.info("************************** Data mart movement from local to s3 for customer data mart **************************")
...
final_sales_team_data_mart_df = s3_customer_store_sales_df_join.select(
   "store_id", "sales_person_id", "sales_person_first_name", "sales_person_last_name", "sales_person_address",
    "sales_person_pincode", "store_manager_name", "manager_id", "is_manager",
...
final_sales_team_data_mart_df.write.format("parquet").option(
    "header", "true").mode(
    "overwrite").partitionBy(
    "sales_month", "store_id").option(
    "path", config.sales_team_data_mart_partitioned_local_file).save()
...
logger.info("****** Calculation fo customer mart table written  ********")

# calculation for sales team mart
# find out the total sales done by each sales person every month
# give top performer 1% incentive of total sales of month
# Rest sales person will get nothing
# write the data into mySQL


logger.info("****** Calculating sales mart table ********")
sales_mart_calculation_table_write(final_sales_team_data_mart_df)
logger.info("****** Calculation for sales mart table written  ********")
...
logger.info("****** Deleting sales data mart partitioned from local  ********")
delete_local_file(config.sales_team_data_mart_partitioned_local_file)
logger.info("****** Deleted sales data mart partitioned from local ********")
